class EvaluationPresenter < BasePresenter
  ...
  def project_status
    ...
    if evaluation.completed_at
      ...
      completed = evaluation.completed_at.to_date.to_s
  ...
  def date_submitted
    ...
    if days > 0
      result = content_tag(:span, curriculum_day , {class: 'badge badge-danger', 'data-toggle': 'tooltip', 'title': "#{days.to_i} days late" })
    else
      result = content_tag(:span, curriculum_day , {class: 'badge badge-success', 'data-toggle': 'tooltip', 'title': "#{days.to_i.abs} days early" })
    end
    result += tag('br')
