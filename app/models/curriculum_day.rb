...
class CurriculumDay
  ...
  def to_s
    ...
    @to_s = if @break_days && @break_days.include?(day_number)
              "w#{week}e"
            elsif day_number <= 0
  ...
  def calculate_date
    ...
    @date = date
    if @curriculum_break && (day_number > @day_break_starts_on)
