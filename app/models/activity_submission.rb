class ActivitySubmission < ApplicationRecord
  ...
  scope :proper, -> {
    joins(:activity).where("activities.type NOT IN ('QuizActivity', 'PinnedNote', 'LecturePlan', 'Breakout', 'TestActivity') AND (activities.evaluates_code = false OR activities.evaluates_code IS NULL OR activity_submissions.finalized = true)")
  }
  ...
  scope :for_week, ->(day) {
    joins(:activity).where("activities.day BETWEEN ? AND ?", day.start_of_week.to_s, day.end_of_week.to_s)
  }
  scope :until_day, ->(day) {
    joins(:activity).where("activities.day <= ?", day.to_s)
  }
