class DayFeedback < ApplicationRecord
  ...
  def self.percentage_of_mood(mood)
    return 0 if count == 0
    count_for_mood(mood).to_f / count * 100
  end

  def self.to_csv
    day_feedback_attributes = %w[day mood text created_at archived_at notes]
  ...
  def self.filter_by(options)
    ...
    options.inject(all) do |result, (k, v)|
      ...
      result = if attribute == 'archived?'
                 filter_by_archived(v, result)
