class SmartQueueRouter::RouterSettings
  ...
  def call
    ...
    program_settings ||= {}
    context.task_penalty = @settings[:task_penalty] || program_settings['task_penalty'] || -2
    context.assistance_penalty = @settings[:assistance_penalty] || program_settings['assistance_penalty'] || -1
    context.evaluation_penalty = @settings[:evaluation_penalty] || program_settings['evaluation_penalty'] || 0
    context.tech_interview_penalty = @settings[:tech_interview_penalty] || program_settings['tech_interview_penalty'] || -3
    ...
    context.max_queue_size = @settings[:max_queue_size] || program_settings['max_queue_size'] || 10
  end
