class QueueStats
  ...
  def assistance_requests
    ...
    requests = requests.where('assistance_requests.start_at >= ?', from_time) if params[:from_date].present?

    requests = requests.where('assistance_requests.start_at <= ?', to_time) if params[:to_date].present?

    requests = requests.where(day: params[:day].split(/\s*,\s*/)) if params[:day].present?

    requests = requests.for_location(params[:location_ids]) if params[:location_ids].present?
