...
describe Scheduled::Curriculum::WeeklyDigest do
  ...
  context 'when there is feedback to report on' do
    ...
    let!(:great_feedback_a)  { create :activity_feedback, activity: activity_a, rating: 5, created_at: 1.day.ago }
    let!(:medium_feedback_a) { create :activity_feedback, activity: activity_a, rating: 3, created_at: 1.day.ago }
    let!(:low_feedback_a)    { create :activity_feedback, activity: activity_a, rating: 2, created_at: 1.day.ago }
    let!(:poor_feedback_a)   { create :activity_feedback, activity: activity_a, rating: 1, created_at: 1.day.ago }
    ...
    let!(:poor_feedback_b) { create :activity_feedback, activity: activity_b, rating: 1, created_at: 1.day.ago }
    let!(:good_feedback_b) { create :activity_feedback, activity: activity_b, rating: 4, created_at: 1.day.ago }
    let!(:poor_feedback_without_details_b) { create :activity_feedback, activity: activity_b, rating: 1, detail: '', created_at: 1.day.ago }

    # Feedback against activity_c (only positive)
    let!(:good_feedback_c) { create :activity_feedback, activity: activity_c, rating: 4, created_at: 1.day.ago }
