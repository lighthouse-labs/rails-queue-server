namespace :import do
  ...
  task webcodereviews: :environment do
    ...
    def find_student(info_row, location)
      ...
      student_email = if location == 'Toronto'
                        info_row[2].to_s
                      else
                        info_row[1].to_s
                      end
      ...
      return if %w[Grochowski Brooks].any? { |last_name| student_last_name.include?(last_name) }
    ...
    CSV.foreach(Rails.public_path.join('tor-web.csv')) do |row|
      # Find student
      if row[0] && row[0].length > 1 && !(%w[January February March Student].any? { |term| row[0].include?(term) })
        ...
        row[3..row.length].each_slice(4) do |review|
          next unless review[0] && !review[0].include?("Code Review") && (review[0] != "Mentor")
    ...
    CSV.foreach(Rails.public_path.join('van-web.csv')) do |row|
      # Find student
      if row[0] && row[0].length > 1 && !(%w[Okanagan January February Feburary October November March Student].any? { |term| row[0].include?(term) })
        student = find_student(row, 'Vancouver')
        if student
          # Find teacher for each code review and create code review
          row[2..row.length].each_slice(4) do |review|
            next unless review[0] && review[0] != '-' && !review[0].include?("Code Review") && (review[0] != "Mentor")
